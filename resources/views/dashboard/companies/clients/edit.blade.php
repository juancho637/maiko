        ...
        <div class="col-12 col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4>{{ ucfirst(__("editar cliente")) }}</h4>
                    <div class="card-header-action">
                        ...
                        <div class="breadcrumb-item">{{ ucfirst(__("editar cliente")) }}</div>
                    </div>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('dashboard.companies.clients.update', [$company, $client]) }}">
                        <div class="row">
                            @csrf
                            @method('PUT')
                            <div class="form-group col-md-12 {{ $errors->has('name') ? 'has-error' : '' }}">
                                <label for="name">{{ ucfirst(__("nombre")) }}</label>
                                <input type="text" id="name" name="name" value="{{ old('name', $client->name) }}" class="form-control" {{ $errors->has('name') || !old('name') ? 'autofocus' : '' }}>
                            ...
                            <div class="form-group col-md-4 {{ $errors->has('country_id') ? 'has-error' : '' }}">
                                ...
                                <select class="form-control" id="country_id" name="country_id" style="width: 100%" {{ $errors->has('country_id') ? 'autofocus' : '' }}>
                                    @foreach($countries as $country)
                                        <option value="{{ $country->id }}" {{ old("country_id", $client->city->state->country_id) == $country->id ? 'selected' : '' }}>{{ ucfirst($country->name) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-4 {{ $errors->has('state_id') ? 'has-error' : '' }}">
                                ...
                                <select id="state_id" name="state_id" class="form-control" style="width: 100%" {{ $errors->has('state_id') ? 'autofocus' : '' }}>
                                    <option value="{{ old('state_id', $client->city->state_id) }}">{{\App\State::where('id', old('state_id', $client->city->state_id))->first()->name}}</option>
                                </select>
                            ...
                            <div class="form-group col-md-4 {{ $errors->has('city_id') ? 'has-error' : '' }}">
                                ...
                                <select id="city_id" name="city_id" class="form-control" style="width: 100%" {{ $errors->has('city_id') ? 'autofocus' : '' }} disabled>
                                    <option value="{{ old('city_id', $client->city_id) }}">{{\App\City::where('id', old('city_id', $client->city_id))->first()->name}}</option>
                                </select>
                                {!! $errors->first('city_id', '<span class="help-block">:message</span>') !!}
                            </div>
                            <div class="form-group col-md-12 {{ $errors->has('address') ? 'has-error' : '' }}">
                                <label for="address">{{ ucfirst(__("dirección")) }}</label>
                                <input type="text" id="address" name="address" value="{{ old('address', $client->address) }}" class="form-control" {{ $errors->has('address') || !old('address') ? 'autofocus' : '' }}>
