                    ...
                    <form method="POST" action="{{ route('dashboard.tanks.update', $tank) }}">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4 {{ $errors->has('long') ? 'has-error' : '' }}">
                                ...
                                <input type="text" id="long" name="long" value="{{ old('long', $tank->long) }}" class="form-control" {{ $errors->has('long') ? 'autofocus' : '' }}>
...
<script>
    $(function () {
        ...
        $('input[name="fabrication_year"]').daterangepicker({
            ...
            showDropdowns: true,
            startDate: "{{ old('fabrication_year', $tank->fabrication_year) }}",
            minYear: 1901,
